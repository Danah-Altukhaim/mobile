...
const STANDING_STYLE: Record<AccountStanding, string> = {
  cleared: 'cck-chip-positive',
  on_track: 'cck-chip-positive',
  hold: 'cck-chip-negative',
};
...
export default function FinancePage() {
  ...
  return (
    <div dir={dir}>
      ...
      <p className="text-xs text-muted mt-6 leading-relaxed">{t('finance.linkedNote')}</p>
        ...
        <tbody>
          {visible.map((a) => (
            <tr
              ...
              className="border-b border-line last:border-0 align-top cursor-pointer hover:bg-canvas focus:outline-none focus-visible:bg-canvas"
            >
              <td className="p-4">
                <p className="font-medium">{locale === 'ar' ? a.name_ar : a.name_en}</p>
                <p className="text-xs text-muted font-mono">{a.student_id}</p>
                <div className="flex flex-wrap gap-1 mt-1.5">
                  ...
                  {a.repeated_course && (
                    <span className="px-1.5 py-0.5 rounded-sm text-[11px] font-medium cck-chip-neutral">
              ...
              <td className="p-4 whitespace-nowrap text-ink">{kwd(a.paid_amount)}</td>
              <td className={`p-4 whitespace-nowrap font-medium ${
                a.balance > 0 && (a.standing === 'hold' || a.installments.some((i) => i.status === 'overdue'))
                  ? 'text-danger-600' : 'text-ink'
              }`}>
                {kwd(a.balance)}
              </td>
              ...
              <td className="p-4">
                <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${STANDING_STYLE[a.standing]}`}>
...
function ClearancesTab({
  ...
  return (
    <div className="space-y-3">
      {clearances.map((c) => (
        <div key={c.id} className="cck-card p-5">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${
              c.outstanding > 0 ? 'cck-chip-negative' : 'cck-chip-positive'
            }`}>
              {c.outstanding > 0
            ...
            {!c.cid_uploaded && (
              <span className="px-2 py-0.5 rounded-sm text-xs font-medium cck-chip-negative">
          ...
          {c.status === 'pending' && (
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              <button
                ...
                className="px-3 py-1.5 border border-danger-200 text-danger-700 rounded-sm text-sm hover:bg-danger-50 disabled:opacity-50"
              >
  ...
  const FeeRow = ({ label, value, note }: { label: string; value: string; note?: string }) => (
    <div className="flex items-center justify-between gap-4 py-2 border-b border-line last:border-0 text-sm">
      <span className="text-body">
        {label}
        {note && (
          <span className="ms-2 px-1.5 py-0.5 rounded-sm text-[10px] font-medium bg-danger-50 text-danger-700 align-middle">
            {note}
          </span>
        )}
      </span>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Section title={t('finance.schedule.tuition')}>
        ...
        <p className="text-xs text-muted mt-3 leading-relaxed">{t('finance.schedule.grantNote')}</p>
      </Section>
      ...
      <Section title={t('finance.schedule.installmentPlan')}>
        ...
        <p className="text-xs text-muted mt-3 leading-relaxed">{t('finance.schedule.installmentNote')}</p>
      </Section>
      ...
      <Section title={t('finance.schedule.methods')}>
        <div className="flex flex-wrap gap-2 mb-5">
          {CCK_PAYMENT_METHODS.map((m) => (
            <span key={m} className="px-2.5 py-1 rounded-sm text-sm bg-canvas text-body">
              {t(`finance.method.${m}`)}
            </span>
          ))}
        </div>
...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2 border-b border-line last:border-0 text-sm">
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white h-full w-full max-w-md shadow-xl overflow-y-auto">
        <div className="flex items-start justify-between gap-4 p-5 border-b border-line sticky top-0 bg-white">
          <div className="min-w-0">
            <p className="text-xs text-muted">{t('finance.detail.title')}</p>
            <h2 className="text-lg font-bold mt-0.5 truncate">{name}</h2>
            <p className="text-xs text-muted font-mono">{account.student_id}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className={`px-2 py-0.5 rounded-sm text-xs font-medium ${STANDING_STYLE[account.standing]}`}>
            ...
            <button
              ...
              className="flex items-center justify-center w-8 h-8 rounded-sm border border-line-strong text-muted hover:bg-canvas focus:outline-none focus:ring-2 focus:ring-pair-500"
            >
        ...
        <div className="p-5 space-y-6">
          <section>
            <h3 className="text-xs font-semibold uppercase text-muted mb-1">{t('finance.detail.contact')}</h3>
            <DetailRow
          ...
          <section>
            <h3 className="text-xs font-semibold uppercase text-muted mb-1">{t('finance.detail.program')}</h3>
          ...
          <section>
            <h3 className="text-xs font-semibold uppercase text-muted mb-1">{t('finance.detail.account')}</h3>
            <DetailRow label={t('finance.col.total')} value={kwd(account.total_payable)} />
            <DetailRow label={t('finance.col.paid')} value={kwd(account.paid_amount)} />
            <DetailRow
              label={t('finance.col.balance')}
              value={<span className={account.balance > 0 ? 'text-danger-600' : 'text-ink'}>{kwd(account.balance)}</span>}
            />
          </section>

          <section>
            <h3 className="text-xs font-semibold uppercase text-muted mb-2">{t('finance.detail.installments')}</h3>
              ...
              {account.installments.map((inst) => (
                <div key={inst.number} className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="flex items-center gap-2 whitespace-nowrap">
                    <span className="text-muted">{t(`finance.detail.instStatus.${inst.status}`)}</span>
          ...
          <section>
            <h3 className="text-xs font-semibold uppercase text-muted mb-2">{t('finance.detail.flags')}</h3>
            {account.late_fee || account.repeated_course ? (
              <div className="flex flex-wrap gap-1.5">
                {account.late_fee && (
                  <span className="px-2 py-1 rounded-sm text-xs font-medium cck-chip-negative">
                    {t('finance.flag.lateFee', { amount: account.late_fee_amount })}
                  </span>
                )}
                {account.repeated_course && (
                  <span className="px-2 py-1 rounded-sm text-xs font-medium cck-chip-neutral">
                    {t('finance.flag.repeated')}
                  </span>
                )}
              </div>
            ) : (
              <p className="text-sm text-muted">{t('finance.detail.noFlags')}</p>
            )}
          </section>
          ...
          <section className="border-t border-line pt-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={sending !== null}
                className="px-3 py-2 border border-pair-600 text-pair-700 rounded-sm text-sm font-medium hover:bg-pair-50 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-pair-500"
              >
                {sending === 'push' ? t('finance.detail.sending') : t('finance.detail.sendPush')}
              </button>
            </div>
            {result && (
              <p className={`mt-3 text-sm px-3 py-2 rounded-sm ${
                result.ok ? 'bg-pair-50 text-pair-700' : 'bg-danger-50 text-danger-700'
              }`}>
                {result.message}
              </p>
            )}
