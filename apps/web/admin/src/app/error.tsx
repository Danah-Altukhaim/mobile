...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div dir={dir} role="alert" className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center">
      ...
      <h1 className="text-lg font-semibold text-ink">{t('common.error')}</h1>
      <p className="text-sm text-muted mt-1 max-w-sm">{t('common.errorDescription')}</p>
      {error.digest && (
        <p className="text-xs text-muted font-mono mt-3">ref: {error.digest}</p>
      )}
      <button
        onClick={reset}
        className="mt-5 px-4 py-2 text-sm bg-pair-600 text-white rounded-sm hover:bg-pair-700"
      >
        {t('common.retry')}
      </button>
    </div>
